...
def calculate_demographic_data(print_data=True):
    ...
    # Coverts age and hours-per-week to numeric values
    df['age'] = pd.to_numeric(df['age'], errors='coerce')
    df['hours-per-week'] = pd.to_numeric(df['hours-per-week'], errors='coerce')

    # Removes unecessary first row
    df = df[1:]
    ...
    highest_earning_country = (countries / df['native-country'].value_counts()).idxmax()
    highest_earning_country_percentage = round((countries / df['native-country'].value_counts()).max() * 100, 1)
